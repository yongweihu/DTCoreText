...
- (void)_registerTagStartHandlers
{
	...
	void (^blockquoteBlock)(void) = ^
	{
        ...
        NSString *allKeys = [[[self->_currentTag styles] allKeys] componentsJoinedByString:@";"];
...
- (void)_registerTagEndHandlers
{
	...
	void (^linkBlock)(void) = ^
	{
		...
		{
			// href中可能包含有空格，所以我们需要处理一下
			NSURL *stylesheetURL = [NSURL URLWithString:[href stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] relativeToURL:self->_baseURL];
